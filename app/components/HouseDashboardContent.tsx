...
function getWarrantyStatus(installedAt: any, warrantyMonths: number | null) {
...
export default function HouseDashboardContent({ data, houseId }: { data: any; houseId: string }) {
  ...
  const thisUtil  = utilityData.find((u: any) => u.month === thisMonth)
  const prevUtil  = utilityData.find((u: any) => u.month === prevMonth)
  const displayUtil = thisUtil ?? utilityData[0] ?? null
  const isCurrentMonth = displayUtil?.month === thisMonth

  const warrantyAlerts = inventoryData.filter((item: any) => {
      ...
      <div id={`tab-container-${houseId}`} data-active-tab="home">
        ...
        <div className="fast-tab fast-tab-home" style={{ padding: '0 16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
            {(() => {
              const alertItems = inventoryData.filter((i: any) => {
              ...
              if (alertItem) {
                ...
                return (
                  <Link href={`/notifications?warranty=1&houseId=${houseId}`} style={{ textDecoration: 'none', color: 'inherit', background: w.bg, border: `0.5px solid ${w.border}`, borderRadius: 14, padding: 14, display: 'block' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 8, background: w.border, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="ti ti-alert-triangle" style={{ fontSize: 16, color: w.color }} /></div>
              ...
              return (
                <TabLink houseId={houseId} tab="inventory" style={{ textDecoration: 'none', color: 'inherit', background: '#0d1f14', border: '0.5px solid #1a3d28', borderRadius: 14, padding: 14, display: 'block' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: '#1a3d28', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="ti ti-shield-check" style={{ fontSize: 16, color: '#34d399' }} /></div>
                    <i className="ti ti-chevron-right" style={{ fontSize: 15, color: '#333', marginTop: 4 }} />
                  </div>
            ...
            <TabLink houseId={houseId} tab="history" style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#1f1000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="ti ti-wallet" style={{ fontSize: 16, color: '#fbbf24' }} /></div>
                <i className="ti ti-chevron-right" style={{ fontSize: 15, color: '#333', marginTop: 4 }} />
              </div>
              <p style={{ fontSize: 13, color: '#666' }}>누적 수리비</p>
              <p style={{ fontSize: 18, fontWeight: 500, color: '#fbbf24', marginTop: 2 }}>{historyData.reduce((s: number, h: any) => s + (h.cost || 0), 0).toLocaleString()}원</p>
              <p style={{ fontSize: 12, color: '#555', marginTop: 1 }}>총 {house._count.histories}건</p>
            </TabLink>

            <TabLink houseId={houseId} tab="inventory" style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#0d1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="ti ti-package" style={{ fontSize: 16, color: '#60a5fa' }} /></div>
                <i className="ti ti-chevron-right" style={{ fontSize: 15, color: '#333', marginTop: 4 }} />
              </div>
              ...
              <p style={{ fontSize: 12, color: '#555', marginTop: 1 }}>{inventoryData.filter((i: any) => getWarrantyStatus(i.installedAt, i.warrantyMonths)).length}개 보증 추적 중</p>
            </TabLink>

            <TabLink houseId={houseId} tab="doctor" style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#0d1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="ti ti-stethoscope" style={{ fontSize: 16, color: '#34d399' }} /></div>
                <i className="ti ti-chevron-right" style={{ fontSize: 15, color: '#333', marginTop: 4 }} />
              </div>
              <p style={{ fontSize: 13, color: '#666' }}>하우스 닥터</p>
              {(() => {
                const unresolved = doctorData.filter((d: any) => !d.resolved).length
                const total = house._count.doctorHistories
                return (
                  <>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 2 }}>
                      {unresolved > 0 ? <p style={{ fontSize: 18, fontWeight: 500, color: '#f87171', margin: 0 }}>미해결 {unresolved}건</p> : <p style={{ fontSize: 18, fontWeight: 500, color: '#34d399', margin: 0 }}>모두 해결</p>}
                      {total > 0 && <span style={{ fontSize: 12, color: '#444' }}>전체 {total}건</span>}
                    </div>
                    <p style={{ fontSize: 12, color: '#555', marginTop: 1 }}>{doctorData[0] ? new Date(doctorData[0].createdAt).toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' }) : '진단 기록 없음'}</p>
                  </>
                )
              })()}
            </TabLink>
            ...
            <div style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 14, padding: 14, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <p style={{ fontSize: 15, color: '#888', textTransform: 'uppercase', letterSpacing: 1 }}>{isCurrentMonth ? '이번달 공과금' : `${displayUtil.month.slice(0, 4)}년 ${displayUtil.month.slice(5)}월 공과금`}</p>
                  {!isCurrentMonth && <span style={{ fontSize: 12, color: '#f97316', background: '#1a0e00', border: '0.5px solid #f97316', borderRadius: 8, padding: '1px 6px' }}>지난달</span>}
                </div>
                <TabLink houseId={houseId} tab="utility" style={{ fontSize: 15, color: '#60a5fa', textDecoration: 'none' }}>전체 보기</TabLink>
              </div>
              <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between' }}>
                {[{ label: '전기', icon: 'ti-bolt', color: '#fbbf24', val: displayUtil.electric }, { label: '수도', icon: 'ti-droplet', color: '#60a5fa', val: displayUtil.water }, { label: '가스', icon: 'ti-flame', color: '#f97316', val: displayUtil.gas }, { label: '통신', icon: 'ti-wifi', color: '#34d399', val: displayUtil.telecom }].map(({ label, icon, color, val }) => (
              ...
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '0.5px solid #1e1e28', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 18, fontWeight: 600, color: '#60a5fa' }}>{((displayUtil.electric || 0) + (displayUtil.water || 0) + (displayUtil.gas || 0) + (displayUtil.telecom || 0)).toLocaleString()}원</span>
              </div>
            </div>
          )}
            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {historyData.map((h: any, idx: number) => {
                  ...
                  return (
                    <TabLink key={h.id} houseId={houseId} tab="history" highlight={h.id} style={{ display: 'flex', gap: 12, paddingBottom: idx < historyData.length - 1 ? 16 : 0, position: 'relative', textDecoration: 'none', color: 'inherit' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <div style={{ width: 8, height: 8, borderRadius: '50%', background: dotColors[h.category] || '#888', flexShrink: 0, marginTop: 4 }} />
                        {idx < historyData.length - 1 && <div style={{ width: 1, flex: 1, background: '#1e1e28', marginTop: 4 }} />}
                      </div>
        ...
        <div className="fast-tab fast-tab-inventory" style={{ padding: '0 16px' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
            <Link href={`/houses/${houseId}/inventory/new`} style={{ background: '#1d4ed8', color: '#fff', padding: '8px 16px', borderRadius: 10, fontSize: 15, textDecoration: 'none', fontWeight: 500 }}>+ 설비 추가</Link>
          </div>
          {inventoryData.length === 0 ? (
            ...
            <SortableInventoryList initialItems={inventoryData} houseId={houseId} />
          )}
        </div>
        ...
        <div className="fast-tab fast-tab-history" style={{ padding: '0 16px' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
            <Link href={`/houses/${houseId}/history/new`} style={{ background: '#1d4ed8', color: '#fff', padding: '8px 16px', borderRadius: 10, fontSize: 15, textDecoration: 'none', fontWeight: 500 }}>+ 이력 추가</Link>
          </div>
          {historyData.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#555' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {historyData.map((h: any) => (
                <HistoryCard key={h.id} h={h} houseId={houseId} deleteAction={deleteHistory.bind(null, h.id, houseId)} />
              ))}
            </div>
          )}
        </div>
        ...
        {/* UTILITY TAB */}
        {(() => {
          const tu = utilityData.find((u: any) => u.month === thisMonth)
          const pu = utilityData.find((u: any) => u.month === prevMonth)
          return (
            <div className="fast-tab fast-tab-utility" style={{ padding: '0 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
                <Link href={`/houses/${houseId}/utility/new`} style={{ background: '#1d4ed8', color: '#fff', padding: '10px 18px', borderRadius: 10, fontSize: 16, textDecoration: 'none', fontWeight: 500 }}>+ 공과금 입력</Link>
              </div>
              {tu ? (
                <div style={{ background: '#0d1a2e', border: '0.5px solid #1e3a5f', borderRadius: 16, padding: 16, marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    {[{ label: '전기세', icon: 'ti-bolt', color: '#fbbf24', val: tu.electric, prev: pu?.electric }, { label: '수도세', icon: 'ti-droplet', color: '#60a5fa', val: tu.water, prev: pu?.water }, { label: '가스비', icon: 'ti-flame', color: '#f97316', val: tu.gas, prev: pu?.gas }, { label: '통신비', icon: 'ti-wifi', color: '#34d399', val: tu.telecom, prev: pu?.telecom }].map(({ label, icon, color, val, prev }) => {
                ...
                <div style={{ textAlign: 'center', padding: '32px 0', color: '#555', marginBottom: 16 }}>
                  <i className="ti ti-bolt" style={{ fontSize: 38, display: 'block', marginBottom: 8 }} /><p style={{ fontSize: 15 }}>이번달 공과금을 입력해보세요</p>
                </div>
              )}
              {utilityData.length > 0 && (
                <>
                  <p style={{ fontSize: 15, color: '#666', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>월별 이력</p>
                  <UtilityChart data={utilityData} thisMonth={thisMonth} />
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {utilityData.map((u: any) => {
                        ...
                        <div key={u.id} style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 14, padding: '14px 16px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                              ...
                              <Link href={`/houses/${houseId}/utility/new?month=${u.month}`} style={{ color: '#444', textDecoration: 'none', fontSize: 20 }}><i className="ti ti-pencil" /></Link>
                              <DeleteUtilityButton utilityId={u.id} houseId={houseId} month={u.month} />
                            </div>
                          </div>
            ...
            <div className="fast-tab fast-tab-valuation" style={{ padding: '0 16px' }}>
              {v && estimatedPrice > 0 ? (
                <>
                  ...
                  <div style={{ background: '#111118', borderRadius: 12, padding: 14, fontSize: 14, color: '#555', lineHeight: 1.8, marginBottom: 16 }}>⚠️ 이 금액은 공시가격 기반 참고용 추정치입니다. 실제 매매가와 다를 수 있습니다.</div>
                </>
              ) : (
                <div style={{ textAlign: 'center', padding: '48px 0', color: '#555' }}>
              ...
              <Link href={`/houses/${houseId}/valuation`} style={{ display: 'block', width: '100%', background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: 14, padding: '15px', fontSize: 17, fontWeight: 500, cursor: 'pointer', textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box' }}>
