...
export default function AiValuation({ address, houseType, buildYear, area }: Props) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {!result && (
        <button
          ...
          style={{
            ...
            padding: '15px', fontSize: 15, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer',
      ...
      {error && (
        <p style={{ color: '#f87171', fontSize: 13, textAlign: 'center', marginTop: 8 }}>{error}</p>
      )}
        ...
        <div style={{ background: 'linear-gradient(135deg, #052e16 0%, #0f2818 100%)', border: '0.5px solid #166534', borderRadius: 20, padding: 20 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <i className="ti ti-sparkles" style={{ fontSize: 16, color: '#4ade80' }} />
              <span style={{ fontSize: 13, color: '#4ade80', fontWeight: 500 }}>AI 시세 추정</span>
            </div>
            <span style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: 20 }}>
            <p style={{ fontSize: 11, color: '#555', marginBottom: 6 }}>추정 시세 범위</p>
            <p style={{ fontSize: 14, color: '#888', marginBottom: 4 }}>
              {fmt(result.minPrice)} ~ {fmt(result.maxPrice)}
            </p>
            <p style={{ fontSize: 36, fontWeight: 700, color: '#4ade80', letterSpacing: -1 }}>
              {fmt(result.midPrice)}
            </p>
            <p style={{ fontSize: 11, color: '#555', marginTop: 4 }}>중간 추정가</p>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
              <span style={{ fontSize: 11, color: '#555' }}>최저 {fmt(result.minPrice)}</span>
              <span style={{ fontSize: 11, color: '#555' }}>최고 {fmt(result.maxPrice)}</span>
            </div>
          </div>

          {/* 추정 근거 */}
          <div style={{ background: '#0a0a0f', borderRadius: 12, padding: 14, marginBottom: 12 }}>
            <p style={{ fontSize: 11, color: '#4ade80', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1 }}>추정 근거</p>
            <p style={{ fontSize: 13, color: '#aaa', lineHeight: 1.7 }}>{result.basis}</p>
          </div>

          <p style={{ fontSize: 11, color: '#555', lineHeight: 1.6 }}>⚠️ {result.note}</p>

          {/* 다시 추정 버튼 */}
          <button
            onClick={estimate}
            style={{ marginTop: 14, width: '100%', background: 'none', border: '0.5px solid #166534', borderRadius: 10, padding: '10px', fontSize: 13, color: '#4ade80', cursor: 'pointer', boxSizing: 'border-box' }}
          >
            다시 추정하기
          </button>
        </div>
