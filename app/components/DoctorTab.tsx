      ...
      if (lines[i - 1]?.includes('숨고 검색') && sogooMatch) {
        ...
        return (
          <a key={i} href={url} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-block', marginTop: 4, background: '#1d4ed8', color: '#fff', padding: '8px 16px', borderRadius: 10, fontSize: 13, textDecoration: 'none' }}>
            숨고에서 "{keyword}" 전문가 찾기 →
          </a>
        )
      }
      if (!line.trim()) return <br key={i} />
      return <p key={i} style={{ fontSize: 13, color: '#ccc', lineHeight: 1.7, margin: '2px 0' }}>{line}</p>
    })
  }
  ...
  return (
    <div style={{ padding: '20px 16px' }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <p style={{ fontSize: 12, color: '#888', marginBottom: 8 }}>사진 (선택)</p>
        <input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handleFile} style={{ display: 'none' }} />
        {preview ? (
          <div style={{ position: 'relative' }}>
            <img src={preview} alt="업로드 사진" style={{ width: '100%', borderRadius: 12, maxHeight: 220, objectFit: 'cover' }} />
            <button onClick={() => { setPreview(null); setImageBase64(null) }}
              style={{ position: 'absolute', top: 8, right: 8, background: '#0a0a0f', border: 'none', color: '#f87171', borderRadius: 20, padding: '4px 10px', cursor: 'pointer', fontSize: 12 }}>
              삭제
            </button>
          </div>
        ) : (
          <button onClick={() => fileRef.current?.click()}
            style={{ width: '100%', background: '#1a1a24', border: '1px dashed #2a2a38', borderRadius: 12, padding: '24px 0', color: '#555', cursor: 'pointer', fontSize: 13 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 12, color: '#888', marginBottom: 8 }}>증상 설명</p>
        <textarea
          value={description}
      ...
      <button onClick={handleSubmit} disabled={loading}
        style={{ width: '100%', background: loading ? '#1a2a4a' : '#1d4ed8', color: '#fff', border: 'none', borderRadius: 14, padding: '14px', fontSize: 15, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer' }}>
        {loading ? 'AI 진단 중...' : '🏥 AI 진단 시작'}
      </button>

      {result && (
        <div style={{ marginTop: 24, background: '#0d1a2e', border: '0.5px solid #1e3a5f', borderRadius: 16, padding: 16 }}>
          <p style={{ fontSize: 14, fontWeight: 600, color: '#34d399', marginBottom: 12 }}>✅ 진단 완료</p>
          {renderResult(result)}
        </div>
      )}
    </div>
