...
export default function DoctorTab({ houseId }: { houseId: string }) {
  ...
  function renderResult(text: string) {
    ...
    return lines.map((line, i) => {
      if (line.startsWith('## ')) {
        return <p key={i} style={{ fontSize: 15, fontWeight: 600, color: '#60a5fa', marginTop: 16, marginBottom: 4 }}>{line.replace('## ', '')}</p>
      }
        ...
        if (matName && line.trim().startsWith('-')) {
          ...
          return (
            <a key={i} href={coupangUrl} target="_blank" rel="noopener noreferrer"
              ...
              <span style={{ fontSize: 14, color: '#ccc' }}>🛒 {mat}</span>
              <span style={{ fontSize: 12, color: '#f97316', fontWeight: 500, flexShrink: 0, marginLeft: 8 }}>쿠팡 →</span>
            </a>
          )
        }
      }
      if (!line.trim()) return <br key={i} />
      return <p key={i} style={{ fontSize: 14, color: '#ccc', lineHeight: 1.7, margin: '2px 0', wordBreak: 'keep-all', overflowWrap: 'break-word' }}>{line}</p>
    })
  }
    ...
    <div style={{ padding: '16px 16px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ background: '#0d1a2e', border: '0.5px solid #1e3a5f', borderRadius: 16, padding: 16, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <i className="ti ti-stethoscope" style={{ color: '#60a5fa', fontSize: 21 }} />
          <span style={{ fontSize: 15, fontWeight: 500, color: '#60a5fa' }}>AI 하우스 닥터</span>
        </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: '#888', marginBottom: 8, marginTop: 0 }}>사진 (선택)</p>
        {preview ? (
          <div style={{ position: 'relative', width: '100%' }}>
            ...
            <button onClick={() => { setPreview(null); setImageBase64(null) }}
              style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(0,0,0,0.7)', border: 'none', color: '#f87171', borderRadius: 20, padding: '4px 10px', cursor: 'pointer', fontSize: 13 }}>
              삭제
            </button>
          </div>
        ) : (
          <label style={{ display: 'block', width: '100%', background: '#1a1a24', border: '1px dashed #2a2a38', borderRadius: 12, padding: '24px 0', color: '#555', cursor: 'pointer', fontSize: 14, boxSizing: 'border-box', textAlign: 'center' }}>
            <i className="ti ti-camera" style={{ fontSize: 25, display: 'block', marginBottom: 6, color: '#444' }} />
            사진 촬영 또는 갤러리에서 선택
      ...
      {/* 증상 설명 */}
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 13, color: '#888', marginBottom: 8, marginTop: 0 }}>증상 설명</p>
        <textarea
          value={description}
      ...
      {error && <p style={{ color: '#f87171', fontSize: 14, marginBottom: 12, wordBreak: 'break-all' }}>{error}</p>}

      <button onClick={handleSubmit} disabled={loading}
        style={{ width: '100%', background: loading ? '#1a2a4a' : '#1d4ed8', color: '#fff', border: 'none', borderRadius: 14, padding: '15px', fontSize: 16, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer', boxSizing: 'border-box' }}>
      ...
      {result && (
        <div style={{ marginTop: 24, overflow: 'hidden' }}>
          ...
          <div style={{ background: '#0d1a2e', border: '0.5px solid #1e3a5f', borderRadius: 16, padding: 16, marginBottom: 12, wordBreak: 'keep-all', overflowWrap: 'break-word' }}>
            <p style={{ fontSize: 15, fontWeight: 600, color: '#34d399', marginBottom: 12, marginTop: 0 }}>✅ 진단 완료</p>
            {renderResult(result)}
          </div>
          ...
          {materials.length > 0 && (
            <div style={{ background: '#1a0f00', border: '0.5px solid #f9731622', borderRadius: 14, padding: 14, marginBottom: 12 }}>
              <p style={{ fontSize: 13, color: '#f97316', marginBottom: 10, fontWeight: 500 }}>🛒 쿠팡에서 자재 바로 구매</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {materials.map((mat, i) => (
                  <a key={i} href={`https://www.coupang.com/np/search?q=${encodeURIComponent(mat)}`}
                    ...
                    <span style={{ fontSize: 13, color: '#f97316', fontWeight: 600, flexShrink: 0 }}>쿠팡 검색 →</span>
                  </a>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(soomgoKeyword + ' DIY 수리 방법')}`}
              ...
              <span style={{ fontSize: 12, color: '#555', textAlign: 'center' }}>유튜브 영상으로 직접 수리하기</span>
            </a>
