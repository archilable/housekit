...
export default function SortableInventoryList({ initialItems, houseId, highlightId }: { initialItems: InventoryItem[], houseId: string, highlightId?: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, userSelect: 'none' }}>
      {saving && <p style={{ fontSize: 13, color: '#60a5fa', textAlign: 'center', padding: '2px 0' }}>순서 저장 중...</p>}
      {items.map((item) => {
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2, flexWrap: 'wrap' }}>
                <p style={{ fontSize: 16, fontWeight: 500 }}>{item.name}</p>
                {w && <span style={{ fontSize: 12, color: w.color, background: w.bg, padding: '1px 6px', borderRadius: 10, border: `0.5px solid ${w.border}` }}>{w.label}</span>}
              </div>
              {item.brand && <p style={{ fontSize: 14, color: '#666' }}>{item.brand} {item.model}</p>}
              {item.installedAt && <p style={{ fontSize: 13, color: '#444', marginTop: 1 }}>설치 {new Date(item.installedAt).toLocaleDateString('ko-KR')}</p>}
              {(item.contactCompany || item.contactPhone) && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5, flexWrap: 'wrap' }}>
                  {item.contactCompany && <span style={{ fontSize: 13, color: '#555' }}>{item.contactCompany}</span>}
                  {item.contactPhone && (
                    <a href={`tel:${item.contactPhone}`} style={{ fontSize: 13, color: '#60a5fa', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 3 }}>
                      <i className="ti ti-phone" style={{ fontSize: 13 }} />{item.contactPhone}
                    </a>
                  )}
                </div>
              )}
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
              <a href={`/houses/${houseId}/inventory/${item.id}/edit`} style={{ color: '#60a5fa', fontSize: 20, padding: 4, textDecoration: 'none' }}>
                <i className="ti ti-pencil" />
              </a>
              <button onClick={() => handleDelete(item.id, item.name)} style={{ background: 'none', border: 'none', color: '#f87171', fontSize: 20, padding: 4, cursor: 'pointer' }}>
