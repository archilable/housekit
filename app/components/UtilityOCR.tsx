    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 14, padding: '16px', fontSize: 15, fontWeight: 500,
      ...
      >
        {loading ? (
          ...
          <>
            <i className="ti ti-camera" style={{ fontSize: 21 }} />
            고지서 사진 찍어서 자동 입력
          </>
        )}
      </button>
      ...
      {memo && (
        <div style={{ marginTop: 10, background: '#0d1f14', border: '0.5px solid #1a3d28', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#34d399' }}>
          ✅ {memo}
        </div>
      )}
      {error && (
        <p style={{ marginTop: 8, fontSize: 13, color: '#f87171', textAlign: 'center' }}>{error}</p>
      )}
    </div>
  )
}
