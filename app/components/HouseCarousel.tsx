...
export default function HouseCarousel({ houses: initialHouses }: { houses: House[] }) {
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ padding: '20px 20px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <span style={{ fontSize: 13, background: '#1e3a5f', color: '#60a5fa', padding: '3px 10px', borderRadius: 20, border: '0.5px solid #2a4a80' }}>
                {house.houseType}
              </span>
              {house.buildYear && (
                <span style={{ fontSize: 13, color: '#444', marginLeft: 8 }}>{house.buildYear}년</span>
              )}
            </div>
            <div style={{ textAlign: 'right' }}>
              <p style={{ fontSize: 30, fontWeight: 700, color: scoreColor, lineHeight: 1 }}>{score}</p>
              <p style={{ fontSize: 12, color: '#555', marginTop: 2 }}>건강점수</p>
            </div>
          </div>
          ...
          <div style={{ padding: '0 20px 20px', textAlign: 'center' }}>
            <p style={{ fontSize: 19, fontWeight: 600, color: '#fff', marginBottom: 2 }}>{house.address}</p>
            {house.addressDetail && <p style={{ fontSize: 15, color: '#555' }}>{house.addressDetail}</p>}

            <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginTop: 16 }}>
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: 20, fontWeight: 600, color: '#a78bfa' }}>{house._count.histories}</p>
                <p style={{ fontSize: 13, color: '#555' }}>이력</p>
              </div>
              <div style={{ width: 1, background: '#1e1e28' }} />
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: 20, fontWeight: 600, color: '#60a5fa' }}>{house._count.inventories}</p>
                <p style={{ fontSize: 13, color: '#555' }}>설비</p>
              </div>
              {(house.exclusiveArea ?? house.area) && (
                <>
                  <div style={{ width: 1, background: '#1e1e28' }} />
                  <div style={{ textAlign: 'center' }}>
                    <p style={{ fontSize: 20, fontWeight: 600, color: '#34d399' }}>{house.exclusiveArea ?? house.area}</p>
                    <p style={{ fontSize: 13, color: '#555' }}>㎡</p>
                  </div>
                </>
              )}
            </div>
      ...
      {/* 스와이프 힌트 */}
      {houses.length > 1 && (
        <p style={{ textAlign: 'center', fontSize: 13, color: '#2a2a38', marginBottom: 12 }}>
          ← 좌우로 밀어서 다른 집 보기 →
        </p>
      )}
      ...
      {houses.length > 1 && (
        <>
          {saving && <p style={{ fontSize: 12, color: '#60a5fa', textAlign: 'center', marginBottom: 4 }}>순서 저장 중...</p>}
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4, position: 'relative' }}>
            {houses.map((h, i) => {
              ...
              return (
                ...
                >
                  <p style={{ fontSize: 13, color: i === current ? '#60a5fa' : '#555', marginBottom: 3 }}>{h.houseType}</p>
                  <p style={{
                    fontSize: 14, fontWeight: 500, color: i === current ? '#fff' : '#888',
                    overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 110,
                  }}>
                    {h.address.replace(/^[가-힣]+(특별시|광역시|도|특별자치시|특별자치도)\s*/, '').split(' ').slice(0, 2).join(' ')}
                  </p>
                  {isDrag && (
                    <p style={{ fontSize: 11, color: '#60a5fa', marginTop: 3 }}>← 드래그 →</p>
                  )}
                </button>
              )
            })}
          </div>
          <p style={{ fontSize: 12, color: '#1e1e28', textAlign: 'center', marginTop: 6 }}>꾹 누르고 드래그하면 순서 변경</p>
        </>
      )}
    </div>
  )
