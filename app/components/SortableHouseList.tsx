...
export default function SortableHouseList({ initialHouses }: { initialHouses: House[] }) {
  ...
  return (
    <div>
      {saving && (
        <p style={{ fontSize: 13, color: '#60a5fa', textAlign: 'center', padding: '4px 0 6px' }}>순서 저장 중...</p>
      )}
          ...
          return (
            ...
            >
              ...
              <div style={{ padding: '10px 16px 0', display: 'flex', justifyContent: 'flex-end' }}>
                <span style={{ fontSize: 12, color: '#333' }}>← 밀어서 순서 변경 →</span>
              </div>
              ...
              <Link href={`/houses/${house.id}`} style={{ display: 'block', padding: '8px 18px 14px', textDecoration: 'none', color: 'inherit' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                      <span style={{ fontSize: 13, background: '#0d1a2e', color: '#60a5fa', padding: '2px 8px', borderRadius: 20, border: '0.5px solid #1e3a5f' }}>
                        {house.houseType}
                      </span>
                      {house.buildYear && <span style={{ fontSize: 13, color: '#555' }}>{house.buildYear}년 건축</span>}
                    </div>
                    <p style={{ fontSize: 18, fontWeight: 500, marginBottom: 2 }}>{house.address}</p>
                    {house.addressDetail && <p style={{ fontSize: 15, color: '#666' }}>{house.addressDetail}</p>}
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ fontSize: 24, fontWeight: 500, color: scoreColor }}>{score}</p>
                    <p style={{ fontSize: 12, color: '#555' }}>건강점수</p>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
                  ...
                  <span style={{ fontSize: 14, color: '#555' }}>
                    <i className="ti ti-tool" style={{ fontSize: 14, marginRight: 4 }} />
              ...
              <div style={{ borderTop: '0.5px solid #1a1a22', display: 'flex' }}>
                <Link href={`/houses/${house.id}`} style={{ flex: 1, padding: '10px 0', textAlign: 'center', fontSize: 14, color: '#60a5fa', textDecoration: 'none' }}>
                  대시보드 열기
                </Link>
