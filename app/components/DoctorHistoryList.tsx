    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
      ...
      <div style={{ background: '#111118', borderRadius: '20px 20px 0 0', padding: '24px 20px', paddingBottom: 'calc(env(safe-area-inset-bottom) + 80px)', width: '100%', maxWidth: 480, marginBottom: 60 }}
        ...
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          <span style={{ fontSize: 43 }}>🎉</span>
          <p style={{ fontSize: 19, fontWeight: 600, color: '#fff', marginTop: 8, marginBottom: 4 }}>해결 완료!</p>
          <p style={{ fontSize: 16, color: '#666', margin: 0 }}>수리 이력에 기록해두면 나중에 참고할 수 있어요</p>
        </div>
        <button onClick={goToHistory}
          style={{ width: '100%', background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: 14, padding: '15px', fontSize: 18, fontWeight: 500, cursor: 'pointer', marginBottom: 10 }}>
          📋 수리 이력에 추가하기
        </button>
        <button onClick={onClose}
          style={{ width: '100%', background: '#1a1a24', color: '#888', border: 'none', borderRadius: 14, padding: '15px', fontSize: 17, cursor: 'pointer' }}>
          괜찮아요
        </button>
      </div>
    </div>
        ...
        <summary style={{ padding: '14px 16px', cursor: 'pointer', listStyle: 'none', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, flexWrap: 'wrap' }}>
              {resolved ? (
                <span style={{ fontSize: 13, color: '#34d399', background: '#34d39922', padding: '2px 7px', borderRadius: 10, flexShrink: 0 }}>✅ 해결됨 {resolvedAt && `· ${resolvedAt}`}</span>
              ) : severity ? (
                <span style={{ fontSize: 13, color: severityColor[severity] || '#888', background: (severityColor[severity] || '#888') + '22', padding: '2px 7px', borderRadius: 10, flexShrink: 0 }}>{severity}</span>
              ) : null}
              <span style={{ fontSize: 14, color: '#555' }}>{dateStr}</span>
            </div>
            <p style={{ fontSize: 16, color: resolved ? '#555' : '#ccc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}>
              {h.description || summary}
            </p>
          </div>
          <i className="ti ti-chevron-down" style={{ fontSize: 19, color: '#444', flexShrink: 0, marginTop: 2 }} />
        </summary>
        ...
        <div style={{ padding: '0 16px 16px', borderTop: '0.5px solid #1e1e28' }}>
          ...
          <div style={{ marginTop: 12, marginBottom: 16 }}>
            {h.result.split('\n').map((line, i) => {
              ...
              if (line.startsWith('## ')) return <p key={i} style={{ fontSize: 16, fontWeight: 600, color: '#60a5fa', marginTop: 12, marginBottom: 4 }}>{line.replace('## ', '')}</p>
              if (!line.trim()) return <br key={i} />
              return <p key={i} style={{ fontSize: 15, color: '#aaa', lineHeight: 1.6, margin: '2px 0', wordBreak: 'keep-all' }}>{line}</p>
            })}
          </div>
          ...
          {materials.length > 0 && (
            <div style={{ background: '#1a0f00', border: '0.5px solid #f9731622', borderRadius: 12, padding: 12, marginBottom: 12 }}>
              <p style={{ fontSize: 14, color: '#f97316', marginBottom: 8, fontWeight: 500 }}>🛒 쿠팡에서 자재 바로 구매</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {materials.map((mat, i) => (
                  <a key={i} href={`https://www.coupang.com/np/search?q=${encodeURIComponent(mat)}`}
                    ...
                    <span style={{ fontSize: 14, color: '#f97316', fontWeight: 600, flexShrink: 0 }}>쿠팡 →</span>
                  </a>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: resolved ? 0 : 10 }}>
            <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(keyword + ' DIY 수리 방법')}`}
              ...
              <span style={{ fontSize: 13, color: '#555', textAlign: 'center' }}>유튜브 영상으로 직접 수리</span>
            </a>
          ...
          {resolved ? (
            <button onClick={handleUnresolve} disabled={loading}
              style={{ width: '100%', background: '#1a1a24', border: '0.5px solid #444', color: '#888', borderRadius: 12, padding: '12px', fontSize: 16, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer', marginTop: 2 }}>
              {loading ? '처리 중...' : '↩ 해결 취소'}
            </button>
          ) : (
            <button onClick={handleResolve} disabled={loading}
              style={{ width: '100%', background: '#0a2010', border: '0.5px solid #34d39944', color: '#34d399', borderRadius: 12, padding: '12px', fontSize: 16, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer', marginTop: 2 }}>
...
export default function DoctorHistoryList({ histories }: { histories: DoctorHistory[] }) {
  ...
  return (
    <div style={{ padding: '0 16px', marginTop: 8 }}>
      <p style={{ fontSize: 14, color: '#444', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>진단 이력</p>
