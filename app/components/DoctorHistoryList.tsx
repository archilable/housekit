...
function ResolveModal({ history, onClose }: { history: DoctorHistory; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
      onClick={onClose}>
      <div style={{ background: '#111118', borderRadius: '20px 20px 0 0', padding: '24px 20px 100px', width: '100%', maxWidth: 480 }}
        onClick={e => e.stopPropagation()}>
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          <span style={{ fontSize: 40 }}>🎉</span>
          <p style={{ fontSize: 16, fontWeight: 600, color: '#fff', marginTop: 8, marginBottom: 4 }}>해결 완료!</p>
          <p style={{ fontSize: 13, color: '#666', margin: 0 }}>수리 이력에 기록해두면 나중에 참고할 수 있어요</p>
        </div>
        <button onClick={goToHistory}
          style={{ width: '100%', background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: 14, padding: '15px', fontSize: 15, fontWeight: 500, cursor: 'pointer', marginBottom: 10 }}>
          📋 수리 이력에 추가하기
        </button>
        <button onClick={onClose}
          style={{ width: '100%', background: '#1a1a24', color: '#888', border: 'none', borderRadius: 14, padding: '15px', fontSize: 14, cursor: 'pointer' }}>
          괜찮아요
        </button>
      </div>
    </div>
      ...
      <details
        ...
        <summary style={{ padding: '14px 16px', cursor: 'pointer', listStyle: 'none', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ fontSize: 13, color: resolved ? '#555' : '#ccc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}>
        ...
        <div style={{ padding: '0 16px 16px', borderTop: '0.5px solid #1e1e28' }}>
          ...
          <div style={{ marginTop: 12, marginBottom: 16 }}>
            {h.result.split('\n').map((line, i) => {
              ...
              if (line.startsWith('## ')) return <p key={i} style={{ fontSize: 13, fontWeight: 600, color: '#60a5fa', marginTop: 12, marginBottom: 4 }}>{line.replace('## ', '')}</p>
