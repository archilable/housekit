  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', padding: '60px 20px 100px' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 23, fontWeight: 700, color: '#fff', margin: 0 }}>내 정보</h1>
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          {user.image ? (
            ...
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#1e1e28', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="ti ti-user" style={{ fontSize: 29, color: '#555' }} />
            </div>
          )}
          <div>
            <p style={{ fontSize: 19, fontWeight: 600, color: '#fff', margin: 0 }}>{user.name ?? '이름 없음'}</p>
            ...
            {dbUser?.accounts && dbUser.accounts.length > 0 && (
              <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
                {dbUser.accounts.map(a => (
                  <span key={a.provider} style={{ fontSize: 12, color: a.provider === 'kakao' ? '#191919' : '#fff', background: a.provider === 'kakao' ? '#FEE500' : '#1d4ed8', borderRadius: 8, padding: '2px 8px', fontWeight: 600 }}>
      ...
      <form action={async () => { 'use server'; await signOut({ redirectTo: '/login' }) }}>
        <button type="submit" style={{
          ...
          borderRadius: 14, padding: '15px', fontSize: 16, fontWeight: 500,
          color: '#f87171', cursor: 'pointer', boxSizing: 'border-box',
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
        }}>
          <i className="ti ti-logout" style={{ fontSize: 19 }} />
          로그아웃
        </button>
      </form>
    </div>
