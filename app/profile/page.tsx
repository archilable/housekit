  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', padding: '60px 20px 100px' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: '#fff', margin: 0 }}>내 정보</h1>
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          {user.image ? (
            ...
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#1e1e28', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="ti ti-user" style={{ fontSize: 30, color: '#555' }} />
            </div>
          )}
          <div>
            <p style={{ fontSize: 20, fontWeight: 600, color: '#fff', margin: 0 }}>{user.name ?? '이름 없음'}</p>
            <p style={{ fontSize: 15, color: '#555', margin: '4px 0 0' }}>
            ...
            {dbUser?.accounts && dbUser.accounts.length > 0 && (
              <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
                {dbUser.accounts.map(a => (
                  <span key={a.provider} style={{ fontSize: 13, color: a.provider === 'kakao' ? '#191919' : '#fff', background: a.provider === 'kakao' ? '#FEE500' : '#1d4ed8', borderRadius: 8, padding: '2px 8px', fontWeight: 600 }}>
      ...
      <form action={async () => { 'use server'; await signOut({ redirectTo: '/login' }) }}>
        <button type="submit" style={{
          ...
          borderRadius: 14, padding: '15px', fontSize: 17, fontWeight: 500,
