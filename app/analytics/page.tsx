  ...
  return (
    <div style={{ padding: '24px 16px 0', width: '100%', boxSizing: 'border-box' }}>
      <div style={{ marginBottom: 20 }}>
        <p style={{ fontSize: 14, color: '#555', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>분석</p>
        <h1 style={{ fontSize: 25, fontWeight: 500 }}>자산 분석</h1>
      </div>
      ...
      {totalAssets > 0 && (
        <div style={{ background: 'linear-gradient(135deg, #0d1a2e 0%, #111828 100%)', border: '0.5px solid #1e3a5f', borderRadius: 20, padding: 20, marginBottom: 24 }}>
          <p style={{ fontSize: 15, color: '#60a5fa', textTransform: 'uppercase', letterSpacing: 2, marginBottom: 10 }}>총 부동산 자산 추정</p>
          <p style={{ fontSize: 37, fontWeight: 700, color: '#fff', letterSpacing: -1, marginBottom: 4 }}>{fmt(totalAssets)}</p>
          <p style={{ fontSize: 15, color: '#555' }}>{houses.filter(h => calcEstimated(h) != null).length}개 주택 합산 · 참고용</p>
            ...
            {houses.map(h => {
              ...
              return (
                <div key={h.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 16, color: '#888', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, marginRight: 8 }}>{h.address}</span>
                  <span style={{ fontSize: 17, fontWeight: 600, color: '#60a5fa', flexShrink: 0 }}>{fmt(est)}</span>
                </div>
              )
            })}
          </div>
      ...
      <p style={{ fontSize: 16, color: '#666', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 16 }}>공과금 분석</p>

      {houses.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 0', color: '#555' }}>
          <i className="ti ti-chart-bar" style={{ fontSize: 43, display: 'block', marginBottom: 8 }} />
          <p>등록된 주택이 없습니다</p>
        </div>
      ) : (
        houses.map(house => {
          ...
          return (
            <div key={house.id} style={{ marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div>
                  <p style={{ fontSize: 18, fontWeight: 500 }}>{house.address}</p>
                  <p style={{ fontSize: 14, color: '#555' }}>{house.houseType}</p>
                </div>
                <Link href={`/houses/${house.id}?tab=utility`} style={{ fontSize: 15, color: '#60a5fa', textDecoration: 'none', background: '#0d1a2e', padding: '6px 12px', borderRadius: 8, border: '0.5px solid #1e3a5f' }}>
                  입력하기
                </Link>
              </div>
              ...
              {thisTotal != null && (
                <div style={{ background: '#0d1a2e', border: '0.5px solid #1e3a5f', borderRadius: 14, padding: 16, marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    <div>
                      <p style={{ fontSize: 14, color: '#60a5fa', marginBottom: 4 }}>이번달 합계</p>
                      <p style={{ fontSize: 27, fontWeight: 600, color: '#fff' }}>{thisTotal.toLocaleString()}원</p>
                    </div>
                    {diff != null && (
                      <div style={{ textAlign: 'right' }}>
                        <p style={{ fontSize: 14, color: '#555', marginBottom: 4 }}>전월 대비</p>
                        <p style={{ fontSize: 19, fontWeight: 500, color: diff > 0 ? '#f87171' : '#34d399' }}>
                      ...
                      return (
                        <div key={label} style={{ background: '#111828', borderRadius: 10, padding: 10 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                            ...
                            {d != null && <span style={{ fontSize: 12, color: d > 0 ? '#f87171' : '#34d399' }}>{d > 0 ? '▲' : '▼'}{Math.abs(d / 1000).toFixed(0)}K</span>}
                          </div>
                          <p style={{ fontSize: 13, color: '#555', marginBottom: 2 }}>{label}</p>
                          <p style={{ fontSize: 16, fontWeight: 500 }}>{val != null ? val.toLocaleString() + '원' : '—'}</p>
                        </div>
                      )
                    })}
                  </div>
              ...
              {house.utilities.length === 0 && (
                <div style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 14, padding: 24, textAlign: 'center', color: '#555' }}>
                  <i className="ti ti-bolt" style={{ fontSize: 31, display: 'block', marginBottom: 6 }} />
                  <p style={{ fontSize: 16 }}>공과금 데이터가 없어요</p>
                </div>
              )}
