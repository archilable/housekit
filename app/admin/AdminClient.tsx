    ...
    <div style={{ padding: '32px 16px 40px', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
        <div style={{
          ...
          <i className="ti ti-shield-lock" style={{ fontSize: 23, color: '#818cf8' }} />
        </div>
        <div>
          <p style={{ fontSize: 12, color: '#555', textTransform: 'uppercase', letterSpacing: 1 }}>관리자</p>
          <h1 style={{ fontSize: 21, fontWeight: 700 }}>HouseKit Admin</h1>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 28 }}>
        <div style={{ background: '#0d1a2e', border: '0.5px solid #1e3a5f', borderRadius: 14, padding: '14px 12px', textAlign: 'center' }}>
          <p style={{ fontSize: 25, fontWeight: 700, color: '#60a5fa' }}>{userList.length}</p>
          <p style={{ fontSize: 12, color: '#666', marginTop: 2 }}>전체 가입자</p>
        </div>
        <div style={{ background: '#0d1f14', border: '0.5px solid #1a3d28', borderRadius: 14, padding: '14px 12px', textAlign: 'center' }}>
          <p style={{ fontSize: 25, fontWeight: 700, color: '#34d399' }}>{todayCount}</p>
          <p style={{ fontSize: 12, color: '#666', marginTop: 2 }}>오늘 가입</p>
        </div>
        <div style={{ background: '#1a0f00', border: '0.5px solid #3d2000', borderRadius: 14, padding: '14px 12px', textAlign: 'center' }}>
          <p style={{ fontSize: 25, fontWeight: 700, color: '#f97316' }}>{totalHouses}</p>
          <p style={{ fontSize: 12, color: '#666', marginTop: 2 }}>등록 자산</p>
        </div>
      </div>

      {/* 가입자 목록 */}
      <p style={{ fontSize: 12, color: '#444', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>가입자 목록</p>
            ...
            <div key={user.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  ...
                  {user.image
                    ...
                    : <i className="ti ti-user" style={{ color: isToday ? '#34d399' : '#60a5fa', fontSize: 19 }} />}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2, flexWrap: 'wrap' }}>
                    <p style={{ fontSize: 15, fontWeight: 500 }}>{user.name || '이름 없음'}</p>
                    {isToday && <span style={{ fontSize: 11, color: '#34d399', background: '#0d1f14', border: '0.5px solid #1a3d28', padding: '1px 6px', borderRadius: 8 }}>NEW</span>}
                    {user.isAdmin && <span style={{ fontSize: 11, color: '#818cf8', background: '#0d0d1f', border: '0.5px solid #2a2a5a', padding: '1px 6px', borderRadius: 8 }}>관리자</span>}
                  </div>
                  <p style={{ fontSize: 13, color: '#555' }}>
                    {user.email ?? (user.accounts.some(a => a.provider === 'kakao') ? '카카오 계정' : '이메일 없음')}
                    {user.accounts.map(a => (
                      <span key={a.provider} style={{ marginLeft: 5, fontSize: 11, color: a.provider === 'kakao' ? '#FEE500' : '#60a5fa', background: '#111', border: '0.5px solid #222', borderRadius: 6, padding: '1px 5px' }}>
                        {a.provider}
                      </span>
                    ))}
                  </p>
                  <p style={{ fontSize: 12, color: '#444', marginTop: 1 }}>{joinDate}</p>
                </div>
                {/* 관리자 권한 토글 */}
                {!isMe && (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12, color: user.isAdmin ? '#818cf8' : '#555',
              ...
              {(user.houses.length > 0 || user.houseAccess.length > 0) && (
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: '0.5px solid #1e1e28', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {user.houses.map(h => (
                    <span key={h.id} style={{ fontSize: 12, color: '#60a5fa' }}>🏠 {h.address}</span>
                  ))}
                  {user.houseAccess.map((a, i) => (
                    <span key={i} style={{ fontSize: 12, color: '#555' }}>🔗 {a.house.address} (공유)</span>
                  ))}
                </div>
              )}
            </div>
