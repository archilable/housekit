    ...
    <div style={{ color: '#fff', width: '100%', boxSizing: 'border-box', overflowX: 'hidden' }}>
      ...
      {warrantyAlerts.length > 0 && (
        <div style={{ margin: '12px 16px 0', background: 'linear-gradient(135deg, #1a0a00, #2d1500)', border: '1px solid #f97316', borderRadius: 14, padding: '14px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <i className="ti ti-bell-ringing" style={{ fontSize: 19, color: '#f97316' }} />
            <p style={{ fontSize: 16, fontWeight: 600, color: '#f97316' }}>보증 만료 임박 알림</p>
            <span style={{ fontSize: 14, background: '#f97316', color: '#000', borderRadius: 10, padding: '1px 7px', fontWeight: 700 }}>{warrantyAlerts.length}</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {warrantyAlerts.map((item: any) => (
              <Link key={item.id} href="/notifications" style={{ textDecoration: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(249,115,22,0.08)', borderRadius: 10, padding: '10px 12px' }}>
                <div>
                  <p style={{ fontSize: 16, fontWeight: 500, color: '#fff' }}>{item.name}</p>
                  {item.brand && <p style={{ fontSize: 14, color: '#888', marginTop: 2 }}>{item.brand}</p>}
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: 16, fontWeight: 700, color: item.diffDays <= 30 ? '#f87171' : '#fbbf24' }}>
                    {item.diffDays <= 30 ? `D-${item.diffDays}` : `${Math.floor(item.diffDays / 30)}개월 후`}
                  </p>
                  <p style={{ fontSize: 13, color: '#666', marginTop: 1 }}>보증 만료</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 16px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 16, background: '#0d1a2e', color: '#60a5fa', padding: '3px 10px', borderRadius: 20, border: '0.5px solid #1e3a5f' }}>
            {house.houseType}
          </span>
          {house.buildYear && <span style={{ fontSize: 16, color: '#555' }}>{house.buildYear}년</span>}
          <Link href={`/houses/${id}/edit`} style={{ color: '#60a5fa', fontSize: 21, textDecoration: 'none' }}>
      ...
      <div style={{ textAlign: 'center', padding: '4px 16px 20px' }}>
        <h1 style={{ fontSize: 25, fontWeight: 500, marginBottom: 4 }}>{house.address}</h1>
        {house.addressDetail && <p style={{ fontSize: 18, color: '#666' }}>{house.addressDetail}</p>}
        {(house.landArea || house.buildArea || house.exclusiveArea) && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 10, flexWrap: 'wrap' }}>
            {house.landArea && <span style={{ fontSize: 16, color: '#555' }}>대지 <span style={{ color: '#aaa' }}>{house.landArea}㎡ ({(house.landArea / 3.305785).toFixed(1)}평)</span></span>}
            {house.buildArea && <span style={{ fontSize: 16, color: '#555' }}>건축 <span style={{ color: '#aaa' }}>{house.buildArea}㎡ ({(house.buildArea / 3.305785).toFixed(1)}평)</span></span>}
            {house.exclusiveArea && <span style={{ fontSize: 16, color: '#60a5fa' }}>전용 <span style={{ fontWeight: 600 }}>{house.exclusiveArea}㎡ ({(house.exclusiveArea / 3.305785).toFixed(1)}평)</span></span>}
          </div>
        )}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginTop: 12 }}>
          <span style={{ fontSize: 15, color: '#666' }}>집 건강점수</span>
          ...
          <span style={{ fontSize: 17, fontWeight: 500, color: scoreColor }}>{score}점</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: '0.5px solid #1e1e28', marginBottom: 16, padding: '0 16px', overflowX: 'auto' }}>
        {[
          ...
          <Link key={t.key} href={`/houses/${id}?tab=${t.key}`} style={{
            flex: 1, textAlign: 'center', padding: '10px 0', fontSize: 18,
      ...
      {tab === 'home' && (
        <div style={{ padding: '0 16px' }}>
          <p style={{ fontSize: 14, color: '#444', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>실시간 현황</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
                  ...
                  <Link href={`/notifications?warranty=1&houseId=${id}`} style={{ textDecoration: 'none', color: 'inherit', background: w.bg, border: `0.5px solid ${w.border}`, borderRadius: 14, padding: 14, display: 'block' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 8, background: w.border, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <i className="ti ti-alert-triangle" style={{ fontSize: 17, color: w.color }} />
                      </div>
                      <i className="ti ti-chevron-right" style={{ fontSize: 16, color: '#333', marginTop: 4 }} />
                    </div>
                    <p style={{ fontSize: 14, color: '#666' }}>보증 알림</p>
                    <p style={{ fontSize: 19, fontWeight: 500, color: w.color, marginTop: 2 }}>{alertItem.name}</p>
                    <p style={{ fontSize: 13, color: '#555', marginTop: 1 }}>{w.label}</p>
                  </Link>
                )
              }
              return (
                <Link href={`/houses/${id}?tab=inventory`} style={{ textDecoration: 'none', color: 'inherit', background: '#0d1f14', border: '0.5px solid #1a3d28', borderRadius: 14, padding: 14, display: 'block' }}>
                  ...
                  <p style={{ fontSize: 14, color: '#666' }}>보증 현황</p>
                  <p style={{ fontSize: 19, fontWeight: 500, color: '#34d399', marginTop: 2 }}>정상</p>
                  <p style={{ fontSize: 13, color: '#555', marginTop: 1 }}>만료 예정 없음</p>
                </Link>
              )
            })()}
            ...
            <Link href={`/houses/${id}?tab=history`} style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#1f1000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className="ti ti-wallet" style={{ fontSize: 17, color: '#fbbf24' }} />
                </div>
                <i className="ti ti-chevron-right" style={{ fontSize: 16, color: '#333', marginTop: 4 }} />
              </div>
              <p style={{ fontSize: 14, color: '#666' }}>누적 수리비</p>
              <p style={{ fontSize: 19, fontWeight: 500, color: '#fbbf24', marginTop: 2 }}>
                {homeHistories.reduce((s: number, h: any) => s + (h.cost || 0), 0).toLocaleString()}원
              </p>
              <p style={{ fontSize: 13, color: '#555', marginTop: 1 }}>총 {house._count.histories}건</p>
            </Link>
            ...
            <Link href={`/houses/${id}?tab=inventory`} style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#0d1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className="ti ti-package" style={{ fontSize: 17, color: '#60a5fa' }} />
                </div>
                <i className="ti ti-chevron-right" style={{ fontSize: 16, color: '#333', marginTop: 4 }} />
              </div>
              <p style={{ fontSize: 14, color: '#666' }}>등록 설비</p>
              <p style={{ fontSize: 19, fontWeight: 500, color: '#60a5fa', marginTop: 2 }}>{house._count.inventories}개</p>
            ...
            <Link href={`/houses/${id}?tab=doctor`} style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#0d1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className="ti ti-stethoscope" style={{ fontSize: 17, color: '#34d399' }} />
                </div>
                <i className="ti ti-chevron-right" style={{ fontSize: 16, color: '#333', marginTop: 4 }} />
              </div>
              <p style={{ fontSize: 14, color: '#666' }}>하우스 닥터</p>
              {(() => {
                ...
                return (
                  <>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 2 }}>
                      {unresolved > 0
                        ? <p style={{ fontSize: 19, fontWeight: 500, color: '#f87171', margin: 0 }}>미해결 {unresolved}건</p>
                        : <p style={{ fontSize: 19, fontWeight: 500, color: '#34d399', margin: 0 }}>모두 해결</p>}
                      {total > 0 && <span style={{ fontSize: 13, color: '#444' }}>전체 {total}건</span>}
                    </div>
                    <p style={{ fontSize: 13, color: '#555', marginTop: 1 }}>
            ...
            <div style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 14, padding: 14, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <p style={{ fontSize: 16, color: '#888', textTransform: 'uppercase', letterSpacing: 1 }}>
                    {isCurrentMonth ? '이번달 공과금' : `${displayUtil.month.slice(0, 4)}년 ${displayUtil.month.slice(5)}월 공과금`}
                  </p>
                  {!isCurrentMonth && (
                    <span style={{ fontSize: 13, color: '#f97316', background: '#1a0e00', border: '0.5px solid #f97316', borderRadius: 8, padding: '1px 6px' }}>지난달</span>
                  )}
                </div>
                <Link href={`/houses/${id}?tab=utility`} style={{ fontSize: 16, color: '#60a5fa', textDecoration: 'none' }}>전체 보기</Link>
              </div>
              <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between' }}>
                {[
                  ...
                  <div key={label} style={{ flex: 1, textAlign: 'center' }}>
                    <i className={`ti ${icon}`} style={{ fontSize: 23, color, display: 'block', marginBottom: 6 }} />
                    <p style={{ fontSize: 14, color: '#666', marginBottom: 3 }}>{label}</p>
                    <p style={{ fontSize: 17, fontWeight: 600, color: '#fff' }}>{val != null ? (val / 1000).toFixed(0) + 'K' : '—'}</p>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '0.5px solid #1e1e28', display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 16, color: '#666' }}>합계</span>
                <span style={{ fontSize: 19, fontWeight: 600, color: '#60a5fa' }}>
          ...
          {homeHistories.length > 0 && (
            <>
              <p style={{ fontSize: 14, color: '#444', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>최근 이력</p>
                      ...
                      <div style={{ flex: 1, paddingBottom: 4 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                            <p style={{ fontSize: 14, color: '#555' }}>{new Date(h.doneAt).toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                            <i className="ti ti-chevron-right" style={{ fontSize: 15, color: '#333' }} />
                          </div>
                        </div>
                        <p style={{ fontSize: 14, color: '#555', marginTop: 1 }}>
                          {h.company && `${h.company} · `}
      ...
      {tab === 'inventory' && (
        <div style={{ padding: '0 16px' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
            <Link href={`/houses/${id}/inventory/new`} style={{ background: '#1d4ed8', color: '#fff', padding: '8px 16px', borderRadius: 10, fontSize: 16, textDecoration: 'none', fontWeight: 500 }}>
              + 설비 추가
            </Link>
          </div>
          {(tabData as any)?.inventories?.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#555' }}>
              <i className="ti ti-package" style={{ fontSize: 43, display: 'block', marginBottom: 8 }} />
              <p>등록된 설비가 없습니다</p>
            </div>
          ) : (
      ...
      {tab === 'history' && (
        <div style={{ padding: '0 16px' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
            <Link href={`/houses/${id}/history/new`} style={{ background: '#1d4ed8', color: '#fff', padding: '8px 16px', borderRadius: 10, fontSize: 16, textDecoration: 'none', fontWeight: 500 }}>
              + 이력 추가
            </Link>
          </div>
          {(tabData as any)?.histories?.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#555' }}>
              <i className="ti ti-clipboard-list" style={{ fontSize: 43, display: 'block', marginBottom: 8 }} />
              <p>등록된 이력이 없습니다</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div key={h.id} id={`history-${h.id}`} style={{ background: isHighlighted ? '#0d1a2e' : 'var(--bg-card)', border: isHighlighted ? '1px solid #3b82f6' : '0.5px solid var(--border)', borderRadius: 14, padding: '14px 16px', display: 'flex', gap: 12, transition: 'border 0.3s' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                        <p style={{ fontSize: 17, fontWeight: 500 }}>{h.title}</p>
                        <p style={{ fontSize: 14, color: '#555', flexShrink: 0, marginLeft: 8 }}>{new Date(h.doneAt).toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                      </div>
                      {h.description && <p style={{ fontSize: 15, color: '#666', marginBottom: 2 }}>{h.description}</p>}
                      <div style={{ display: 'flex', gap: 10, fontSize: 14, color: '#555', flexWrap: 'wrap' }}>
                      ...
                      {(h.estimateImageBase64 || h.contractImageBase64) && (
                        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                          {h.estimateImageBase64 && <div style={{ fontSize: 13, color: '#a78bfa', background: '#1a1040', borderRadius: 6, padding: '3px 8px', display: 'flex', alignItems: 'center', gap: 4 }}><i className="ti ti-file-invoice" style={{ fontSize: 14 }} />견적서</div>}
                          {h.contractImageBase64 && <div style={{ fontSize: 13, color: '#34d399', background: '#0d1f14', borderRadius: 6, padding: '3px 8px', display: 'flex', alignItems: 'center', gap: 4 }}><i className="ti ti-file-text" style={{ fontSize: 14 }} />계약서</div>}
                        </div>
                      )}
                    </div>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <a href={`/houses/${id}/history/${h.id}/edit`} style={{ background: 'none', border: 'none', color: '#60a5fa', cursor: 'pointer', fontSize: 21, padding: 4, textDecoration: 'none' }}>
                        <i className="ti ti-pencil" />
                      </a>
                      <form action={deleteHistory.bind(null, h.id, id)}>
                        <button type="submit" style={{ background: 'none', border: 'none', color: '#f87171', cursor: 'pointer', fontSize: 21, padding: 4 }}>
        ...
        return (
          <div style={{ padding: '0 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
              <Link href={`/houses/${id}/utility/new`} style={{ background: '#1d4ed8', color: '#fff', padding: '10px 18px', borderRadius: 10, fontSize: 17, textDecoration: 'none', fontWeight: 500 }}>
                + 공과금 입력
              </Link>
            </div>
            {tu ? (
              <div style={{ background: '#0d1a2e', border: '0.5px solid #1e3a5f', borderRadius: 16, padding: 16, marginBottom: 16 }}>
                <p style={{ fontSize: 16, color: '#60a5fa', marginBottom: 14, textTransform: 'uppercase', letterSpacing: 1 }}>{thisMonth.replace('-', '년 ')}월 현황</p>
                    ...
                    return (
                      <div key={label} style={{ background: '#111828', borderRadius: 12, padding: 12 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                          ...
                          {diff != null && <span style={{ fontSize: 15, color: diff > 0 ? '#f87171' : '#34d399' }}>{diff > 0 ? '▲' : '▼'}{Math.abs(diff).toLocaleString()}</span>}
                        </div>
                        <p style={{ fontSize: 16, color: '#666', marginBottom: 4 }}>{label}</p>
                        <p style={{ fontSize: 20, fontWeight: 600, color: '#fff' }}>{val != null ? val.toLocaleString() + '원' : '—'}</p>
                      </div>
                    )
                  })}
                </div>
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: '0.5px solid #1e3a5f', display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 17, color: '#666' }}>이번달 합계</span>
                  <span style={{ fontSize: 21, fontWeight: 700, color: '#60a5fa' }}>{((tu.electric || 0) + (tu.water || 0) + (tu.gas || 0) + (tu.telecom || 0)).toLocaleString()}원</span>
                </div>
              </div>
            ) : (
              <div style={{ textAlign: 'center', padding: '32px 0', color: '#555', marginBottom: 16 }}>
                <i className="ti ti-bolt" style={{ fontSize: 39, display: 'block', marginBottom: 8 }} />
                <p style={{ fontSize: 16 }}>이번달 공과금을 입력해보세요</p>
              </div>
            )}
            {utilities.length > 0 && (
              <>
                <p style={{ fontSize: 16, color: '#666', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>월별 이력</p>
                    ...
                    return (
                      <div key={u.id} style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 14, padding: '14px 16px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                          <span style={{ fontSize: 19, fontWeight: 500 }}>{u.month.replace('-', '년 ')}월</span>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            <span style={{ fontSize: 19, fontWeight: 600, color: '#60a5fa' }}>{total.toLocaleString()}원</span>
                            <Link href={`/houses/${id}/utility/new?month=${u.month}`} style={{ color: '#444', textDecoration: 'none', fontSize: 21 }}><i className="ti ti-pencil" /></Link>
                            <DeleteUtilityButton utilityId={u.id} houseId={id} month={u.month} />
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: 12, fontSize: 16, color: '#666' }}>
      ...
      {tab === 'valuation' && (() => {
        ...
        return (
          <div style={{ padding: '0 16px' }}>
            {v && estimatedPrice > 0 ? (
              <>
                <div style={{ background: 'linear-gradient(135deg, #0d1a2e 0%, #111828 100%)', border: '0.5px solid #1e3a5f', borderRadius: 20, padding: 24, marginBottom: 16, textAlign: 'center' }}>
                  <p style={{ fontSize: 15, color: '#60a5fa', textTransform: 'uppercase', letterSpacing: 2, marginBottom: 12 }}>추정 시세</p>
                  <p style={{ fontSize: 41, fontWeight: 700, color: '#fff', letterSpacing: -1, marginBottom: 6 }}>{fmt(estimatedPrice)}</p>
                  <p style={{ fontSize: 15, color: '#555' }}>{new Date().getFullYear()}년 기준 · 참고용 추정치</p>
                  {!isApt && (landVal > 0 || buildVal > 0) && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 20 }}>
                      <div style={{ background: '#0a0a0f', borderRadius: 14, padding: 14 }}>
                        <i className="ti ti-map-pin" style={{ fontSize: 21, color: '#fbbf24', display: 'block', marginBottom: 6 }} />
                        <p style={{ fontSize: 14, color: '#666', marginBottom: 4 }}>토지가</p>
                        <p style={{ fontSize: 20, fontWeight: 600, color: '#fbbf24' }}>{fmt(landVal)}</p>
                      </div>
                      <div style={{ background: '#0a0a0f', borderRadius: 14, padding: 14 }}>
                        <i className="ti ti-home-2" style={{ fontSize: 21, color: '#a78bfa', display: 'block', marginBottom: 6 }} />
                        <p style={{ fontSize: 14, color: '#666', marginBottom: 4 }}>건물가</p>
                        <p style={{ fontSize: 20, fontWeight: 600, color: '#a78bfa' }}>{fmt(buildVal)}</p>
                      </div>
                    </div>
                  )}
                </div>
                <div style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 16, padding: 16, marginBottom: 16 }}>
                  <p style={{ fontSize: 15, color: '#555', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 14 }}>계산 근거</p>
                  {isApt ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#888' }}>공동주택 공시가격</span><span style={{ fontSize: 17, fontWeight: 500 }}>{fmt(v.officialPrice!)}</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#888' }}>공시가격 반영률</span><span style={{ fontSize: 17, fontWeight: 500 }}>{((v.priceRatio ?? 0.70) * 100).toFixed(0)}%</span></div>
                      <div style={{ height: 1, background: '#1e1e28' }} />
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#60a5fa' }}>추정 시세</span><span style={{ fontSize: 18, fontWeight: 700, color: '#60a5fa' }}>{fmt(estimatedPrice)}</span></div>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#888' }}>공시지가</span><span style={{ fontSize: 17 }}>{(v.landPrice ?? 0).toLocaleString()}원/㎡</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#888' }}>대지면적 × 지분율</span><span style={{ fontSize: 17 }}>{v.landArea}㎡ × {((v.landShare ?? 1) * 100).toFixed(0)}%</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#fbbf24' }}>토지가</span><span style={{ fontSize: 17, fontWeight: 600, color: '#fbbf24' }}>{fmt(landVal)}</span></div>
                      <div style={{ height: 1, background: '#1e1e28' }} />
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#888' }}>건축비 단가</span><span style={{ fontSize: 17 }}>{(v.buildCostPerSqm ?? 0).toLocaleString()}원/㎡</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#888' }}>연면적</span><span style={{ fontSize: 17 }}>{v.buildArea}㎡</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#888' }}>감가 ({age}년 × {((v.deprRate ?? 0.02) * 100).toFixed(1)}%)</span><span style={{ fontSize: 17 }}>잔존율 {(Math.max(1 - (v.deprRate ?? 0.02) * age, 0.2) * 100).toFixed(0)}%</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 17, color: '#a78bfa' }}>건물가</span><span style={{ fontSize: 17, fontWeight: 600, color: '#a78bfa' }}>{fmt(buildVal)}</span></div>
                      <div style={{ height: 1, background: '#1e1e28' }} />
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 18, color: '#60a5fa', fontWeight: 500 }}>추정 시세 합계</span><span style={{ fontSize: 19, fontWeight: 700, color: '#60a5fa' }}>{fmt(estimatedPrice)}</span></div>
                    </div>
                  )}
                </div>
                <div style={{ background: '#111118', borderRadius: 12, padding: 14, fontSize: 15, color: '#555', lineHeight: 1.8, marginBottom: 16 }}>
                  ⚠️ 이 금액은 공시가격 기반 참고용 추정치입니다. 실제 매매가와 다를 수 있습니다.
                </div>
              </>
            ) : (
              <div style={{ textAlign: 'center', padding: '48px 0', color: '#555' }}>
                <i className="ti ti-building-estate" style={{ fontSize: 47, display: 'block', marginBottom: 12, color: '#2a2a38' }} />
                <p style={{ fontSize: 18, marginBottom: 6 }}>시세 정보가 없어요</p>
                <p style={{ fontSize: 16, marginBottom: 24, color: '#444' }}>공시지가 등 정보를 입력하면 추정 시세를 계산해드려요</p>
              </div>
            )}
            ...
            <Link href={`/houses/${id}/valuation`} style={{ display: 'block', width: '100%', background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: 14, padding: '15px', fontSize: 18, fontWeight: 500, cursor: 'pointer', textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box' }}>
