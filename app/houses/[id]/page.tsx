        ...
        <div style={{ padding: '0 16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
                  ...
                  <Link href={`/notifications?warranty=1&houseId=${id}`} style={{ textDecoration: 'none', color: 'inherit', background: w.bg, border: `0.5px solid ${w.border}`, borderRadius: 14, padding: 14, display: 'block' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 8, background: w.border, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <i className="ti ti-alert-triangle" style={{ fontSize: 14, color: w.color }} />
                      </div>
                ...
                <Link href={`/houses/${id}?tab=inventory`} style={{ textDecoration: 'none', color: 'inherit', background: '#0d1f14', border: '0.5px solid #1a3d28', borderRadius: 14, padding: 14, display: 'block' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: '#1a3d28', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <i className="ti ti-shield-check" style={{ fontSize: 14, color: '#34d399' }} />
                    </div>
                    <i className="ti ti-chevron-right" style={{ fontSize: 13, color: '#333', marginTop: 4 }} />
                  </div>
            ...
            <Link href={`/houses/${id}?tab=history`} style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#1f1000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className="ti ti-wallet" style={{ fontSize: 14, color: '#fbbf24' }} />
                </div>
                <i className="ti ti-chevron-right" style={{ fontSize: 13, color: '#333', marginTop: 4 }} />
              </div>
              ...
              <p style={{ fontSize: 10, color: '#555', marginTop: 1 }}>총 {house._count.histories}건</p>
            </Link>
            ...
            <Link href={`/houses/${id}?tab=inventory`} style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#0d1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className="ti ti-package" style={{ fontSize: 14, color: '#60a5fa' }} />
                </div>
                <i className="ti ti-chevron-right" style={{ fontSize: 13, color: '#333', marginTop: 4 }} />
              </div>
              <p style={{ fontSize: 11, color: '#666' }}>등록 설비</p>
              <p style={{ fontSize: 16, fontWeight: 500, color: '#60a5fa', marginTop: 2 }}>{house._count.inventories}개</p>
            ...
            <Link href={`/houses/${id}?tab=doctor`} style={{ textDecoration: 'none', color: 'inherit', background: 'var(--bg-card)', border: '0.5px solid var(--border)', borderRadius: 14, padding: 14, display: 'block' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#0d1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className="ti ti-stethoscope" style={{ fontSize: 14, color: '#34d399' }} />
                </div>
                <i className="ti ti-chevron-right" style={{ fontSize: 13, color: '#333', marginTop: 4 }} />
              </div>
            ...
            <div style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 14, padding: 14, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {!isCurrentMonth && (
                    <span style={{ fontSize: 10, color: '#f97316', background: '#1a0e00', border: '0.5px solid #f97316', borderRadius: 8, padding: '1px 6px' }}>지난달</span>
                  )}
                </div>
                      ...
                      <div style={{ flex: 1, paddingBottom: 4 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                            <p style={{ fontSize: 11, color: '#555' }}>{new Date(h.doneAt).toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
      ...
      {tab === 'history' && (
        <div style={{ padding: '0 16px' }}>
          ...
          {(tabData as any)?.histories?.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#555' }}>
              <i className="ti ti-clipboard-list" style={{ fontSize: 40, display: 'block', marginBottom: 8 }} />
              <p>등록된 이력이 없습니다</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div key={h.id} id={`history-${h.id}`} style={{ background: isHighlighted ? '#0d1a2e' : 'var(--bg-card)', border: isHighlighted ? '1px solid #3b82f6' : '0.5px solid var(--border)', borderRadius: 14, padding: '14px 16px', display: 'flex', gap: 12, transition: 'border 0.3s' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                        ...
                        <p style={{ fontSize: 11, color: '#555', flexShrink: 0, marginLeft: 8 }}>{new Date(h.doneAt).toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                      </div>
                      ...
                      {(h.estimateImageBase64 || h.contractImageBase64) && (
                        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                          {h.estimateImageBase64 && <div style={{ fontSize: 10, color: '#a78bfa', background: '#1a1040', borderRadius: 6, padding: '3px 8px', display: 'flex', alignItems: 'center', gap: 4 }}><i className="ti ti-file-invoice" style={{ fontSize: 11 }} />견적서</div>}
                          {h.contractImageBase64 && <div style={{ fontSize: 10, color: '#34d399', background: '#0d1f14', borderRadius: 6, padding: '3px 8px', display: 'flex', alignItems: 'center', gap: 4 }}><i className="ti ti-file-text" style={{ fontSize: 11 }} />계약서</div>}
                        </div>
                      )}
                    </div>
            ...
            {tu ? (
              <div style={{ background: '#0d1a2e', border: '0.5px solid #1e3a5f', borderRadius: 16, padding: 16, marginBottom: 16 }}>
                    ...
                    return (
                      <div key={label} style={{ background: '#111828', borderRadius: 12, padding: 12 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                          ...
                          {diff != null && <span style={{ fontSize: 12, color: diff > 0 ? '#f87171' : '#34d399' }}>{diff > 0 ? '▲' : '▼'}{Math.abs(diff).toLocaleString()}</span>}
                        </div>
                        <p style={{ fontSize: 13, color: '#666', marginBottom: 4 }}>{label}</p>
                        <p style={{ fontSize: 17, fontWeight: 600, color: '#fff' }}>{val != null ? val.toLocaleString() + '원' : '—'}</p>
                      </div>
                    )
                  })}
                </div>
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: '0.5px solid #1e3a5f', display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontSize: 18, fontWeight: 700, color: '#60a5fa' }}>{((tu.electric || 0) + (tu.water || 0) + (tu.gas || 0) + (tu.telecom || 0)).toLocaleString()}원</span>
                </div>
              </div>
            ) : (
                      ...
                      <div key={u.id} style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 14, padding: '14px 16px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            <Link href={`/houses/${id}/utility/new?month=${u.month}`} style={{ color: '#444', textDecoration: 'none', fontSize: 18 }}><i className="ti ti-pencil" /></Link>
      ...
      {tab === 'valuation' && (() => {
        ...
        return (
          <div style={{ padding: '0 16px' }}>
                ...
                <div style={{ background: '#111118', border: '0.5px solid #1e1e28', borderRadius: 16, padding: 16, marginBottom: 16 }}>
                  ...
                  {isApt ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#888' }}>공동주택 공시가격</span><span style={{ fontSize: 14, fontWeight: 500 }}>{fmt(v.officialPrice!)}</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#888' }}>공시가격 반영률</span><span style={{ fontSize: 14, fontWeight: 500 }}>{((v.priceRatio ?? 0.70) * 100).toFixed(0)}%</span></div>
                      <div style={{ height: 1, background: '#1e1e28' }} />
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#60a5fa' }}>추정 시세</span><span style={{ fontSize: 15, fontWeight: 700, color: '#60a5fa' }}>{fmt(estimatedPrice)}</span></div>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#888' }}>공시지가</span><span style={{ fontSize: 14 }}>{(v.landPrice ?? 0).toLocaleString()}원/㎡</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#888' }}>대지면적 × 지분율</span><span style={{ fontSize: 14 }}>{v.landArea}㎡ × {((v.landShare ?? 1) * 100).toFixed(0)}%</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#fbbf24' }}>토지가</span><span style={{ fontSize: 14, fontWeight: 600, color: '#fbbf24' }}>{fmt(landVal)}</span></div>
                      <div style={{ height: 1, background: '#1e1e28' }} />
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#888' }}>건축비 단가</span><span style={{ fontSize: 14 }}>{(v.buildCostPerSqm ?? 0).toLocaleString()}원/㎡</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#888' }}>연면적</span><span style={{ fontSize: 14 }}>{v.buildArea}㎡</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#888' }}>감가 ({age}년 × {((v.deprRate ?? 0.02) * 100).toFixed(1)}%)</span><span style={{ fontSize: 14 }}>잔존율 {(Math.max(1 - (v.deprRate ?? 0.02) * age, 0.2) * 100).toFixed(0)}%</span></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 14, color: '#a78bfa' }}>건물가</span><span style={{ fontSize: 14, fontWeight: 600, color: '#a78bfa' }}>{fmt(buildVal)}</span></div>
                      <div style={{ height: 1, background: '#1e1e28' }} />
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 15, color: '#60a5fa', fontWeight: 500 }}>추정 시세 합계</span><span style={{ fontSize: 16, fontWeight: 700, color: '#60a5fa' }}>{fmt(estimatedPrice)}</span></div>
                    </div>
                  )}
                </div>
            ...
            <Link href={`/houses/${id}/valuation`} style={{ display: 'block', width: '100%', background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: 14, padding: '15px', fontSize: 15, fontWeight: 500, cursor: 'pointer', textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box' }}>
