  ...
  if (houses.length === 0) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh', textAlign: 'center', padding: '0 24px' }}>
        ...
        <h1 style={{ fontSize: 25, fontWeight: 500, marginBottom: 8 }}>집의 이력서를 시작하세요</h1>
        <p style={{ fontSize: 17, color: '#666', marginBottom: 32, lineHeight: 1.6 }}>
          보일러 교체일, 누수 이력, 수리업체까지<br />집의 모든 기록을 한 곳에
        </p>
        <Link href="/houses/new" style={{
          background: '#1d4ed8', color: '#fff', padding: '14px 32px',
          borderRadius: 14, fontSize: 18, fontWeight: 500, textDecoration: 'none',
        }}>
          첫 번째 주택 등록하기
        </Link>
      </div>
  ...
  return (
    <div style={{ padding: '24px 16px 0', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <p style={{ fontSize: 15, color: '#555', marginBottom: 2 }}>내 자산</p>
          <h1 style={{ fontSize: 25, fontWeight: 600 }}>주택 {houses.length}채</h1>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <Link href="/houses/new" style={{
            fontSize: 15, color: '#60a5fa', background: '#0d1a2e',
          ...
          <form action={async () => { 'use server'; await signOut({ redirectTo: '/login' }) }}>
            <button type="submit" style={{ fontSize: 15, color: '#555', background: 'none', border: 'none', cursor: 'pointer', padding: '7px 4px' }}>
              로그아웃
            </button>
          </form>
        </div>
