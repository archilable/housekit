  ...
  if (houses.length === 0) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh', textAlign: 'center', padding: '0 24px' }}>
        ...
        <h1 style={{ fontSize: 23, fontWeight: 500, marginBottom: 8 }}>집의 이력서를 시작하세요</h1>
        ...
        <Link href="/houses/new" style={{
          ...
          borderRadius: 14, fontSize: 16, fontWeight: 500, textDecoration: 'none',
        }}>
          첫 번째 주택 등록하기
        </Link>
      </div>
  ...
  return (
    <div style={{ padding: '24px 16px 0', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <p style={{ fontSize: 13, color: '#555', marginBottom: 2 }}>내 자산</p>
          <h1 style={{ fontSize: 23, fontWeight: 600 }}>주택 {houses.length}채</h1>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <Link href="/houses/new" style={{
            fontSize: 13, color: '#60a5fa', background: '#0d1a2e',
          ...
          <form action={async () => { 'use server'; await signOut({ redirectTo: '/login' }) }}>
            <button type="submit" style={{ fontSize: 13, color: '#555', background: 'none', border: 'none', cursor: 'pointer', padding: '7px 4px' }}>
              로그아웃
            </button>
          </form>
        </div>
